...
template<class T>
T* BinarySearchTree<T>::foundValue(node<T>* p, T& item){
	if(p==0)
		return 0;

	if(item>p->data){ // item greater than root, go and search right side of the tree
		return foundValue(p->right, item);
	}

	else if(item<p->data){ // item smaller than root, go and search left side of the tree
		return foundValue(p->left, item);
	}
	return &p->data;
}
